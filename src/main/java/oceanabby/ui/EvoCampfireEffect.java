...
public class EvoCampfireEffect extends AbstractGameEffect {
    private static final String[] TEXT = CardCrawlGame.languagePack.getUIString(makeID("EvoCampfireEffect")).TEXT;
    private static final float DUR = 1.5f;

    private boolean openedScreen = false;
    private Color color = AbstractDungeon.fadeColor.cpy();

    public EvoCampfireEffect() {
        duration = DUR;
        color.a = 0f;
    ...
    public static class EvoingField {
        public static SpireField<Boolean> evoing = new SpireField<>(() -> false);
    }
    ...
    public static class AttunePreview {
        ...
        public static void Insert(GridCardSelectScreen __instance) {
            if (EvoingField.evoing.get(__instance)) {
    ...
    public static class NoLongerAttuning {
        public static void Prefix(GridCardSelectScreen __instance) {
            EvoingField.evoing.set(__instance, false);
        }
    }
    ...
    public void update() {
        if (!AbstractDungeon.isScreenUp || !EvoingField.evoing.get(AbstractDungeon.gridSelectScreen)) {
        ...
        if (duration < 1.0f && !openedScreen) {
            openedScreen = true;
            AbstractDungeon.gridSelectScreen.open(EvoCampfireOption.getAttunableCards(), 1, TEXT[0], true);
            EvoingField.evoing.set(AbstractDungeon.gridSelectScreen, true);
        }
        if (duration < 0f) {
            isDone = true;
